...
const dragged: Map<string, Node> = new Map();
const allowedToEffect: { [key in DataTransfer['effectAllowed']]: DataTransfer['dropEffect'] } = {
    all: 'copy',
    copy: 'copy',
    move: 'move',
    none: 'none',
...
function getReference(node: Node, x: number, y: number): Reference
{
    const offset = 25;
...
export function draggable(target: Node, scope: string, config: DragDraggableConfig)
{
    ...
    target.on(query, {
        ...
        dragstart: (e: DragEvent, t: Element) => {
            ...
            if(value !== undefined && e.dataTransfer !== null)
            {
                ...
                dragged.set(scope, e.target! as Node);
            }
        },
    });
}

export function on(target: Node, scope: string, config: DragOnConfig)
{
    let valid = false;
    const last = { x: 0, y: 0 };
